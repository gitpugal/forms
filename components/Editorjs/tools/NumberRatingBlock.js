...
class NumberRatingBlock {
    constructor({ data, api, readOnly }) {
        this.api = api;
        this.data = {
            ...data,
            label: data?.label ? data?.label : "Rate your experience",
        };
        this.wrapper = null;
                ...
                <div className="flex items-center">
                    <div className="flex flex-row flex-wrap items-center justify-start gap-2">
                        {Array.from({ length: 10 }).map((_, index) => (
                            <p className="border rounded-md md:h-10 h-7 md:w-10 w-7 cursor-pointer flex items-center justify-center">{index + 1}</p>
                        ))}
                    </div>
                    <div
                        onClick={() => this._toggleTune()}
                        className="absolute required-indicator cursor-pointer  inset-y-0 right-0 top-1/2 -translate-y-1/2 flex items-center pr-3 text-2xl opacity-80 text-red-500"
                    >
                        {this.data.required ? "*" : ""}
...
export default NumberRatingBlock;
